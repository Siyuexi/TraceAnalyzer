...
preserved as ``src-backup/``. Bonus-map construction still reuses the old SWE
trace utilities for now, so these scripts need both directories importable.
"""
...
def ensure_paths() -> None:
    for path in (SRC_ROOT, SRC_BACKUP):
        path_str = str(path)
